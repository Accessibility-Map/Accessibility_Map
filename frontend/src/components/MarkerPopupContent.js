...
const MarkerPopupContent = ({
  ...
  useEffect(() => {
    const handleResizeWindow = () => {
      ...
      window.innerHeight <= 700 ? setShortScreen(true) : setShortScreen(false)
    }
  ...
  const fetchFeaturesAndImages = async () => {
    try {
      const response = await axios.get(
        `${process.env.REACT_APP_API_URL}api/features/location/${location.locationID}`
      )
        ...
        <div
          ...
          <Box sx={{height: '48px', flexGrow: 0, flexShrink: 0}}>
            <Tabs onChange={handleTabChange} value={tab} variant='fullWidth'>
              <Tab style={{fontSize: 11}} label='Description' value='1' />
              <Tab style={{fontSize: 11}} label='Accomodations' value='2' />
              <Tab style={{fontSize: 11}} label='Comments' value='3' />
            </Tabs>
          </Box>
          ...
          <Box sx={{flexGrow: 1, flexShrink: 1, maxHeight: '70%'}}>
            <Box
              hidden={tab != 1}
              sx={{height: '100%', display: tab == 1 ? 'flex' : 'none', flexDirection: 'column'}}>
              ...
              <Typography variant='subtitle2' sx={{margin: '0 !important', textAlign: 'center'}}>
                {averageRating}/5.00 - Average User Rating
              </Typography>
            ...
            <Box hidden={tab != 3} sx={{height: '108%'}}>
              <CommentList
                ...
                userID={userID}
                user={user}></CommentList>
            </Box>
          </Box>

          <Box sx={{height: 'fit-content', flexGrow: 0, flexShrink: 0, marginTop: 'auto'}}>
            <Box
              sx={{height: '40px', marginBottom: 'auto', marginTop: '10px'}}
              hidden={tab != 1 && tab != 2}>
              <Button variant='contained' onClick={handleEditLocation} fullWidth>
                Edit Location
              </Button>
            </Box>
